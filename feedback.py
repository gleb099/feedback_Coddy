...
if workk == "1":
	...
	dataUpload = int(input("Какую выгрузку делаем?(Введи цифру)\n1. Прошло два занятия \n2. Перед последним занятием\n"))
			...
			if dataUnitStudents[j]["EdUnitOfficeOrCompanyName"] in branchs:
				if data1[i]["Id"] == dataUnitStudents[j]["EdUnitId"]:
					...
					if dataUpload == 1:
						...
						if (dataUnitStudents[j]["Days"][0]["Date"] in tempDates or dataUnitStudents[j]["Days"][1]["Date"] in tempDates):

							if len(dataUnitStudents[j]['Days']) >= 2:

								if ((dataUnitStudents[j]["Days"][0]["Pass"] == False) or (dataUnitStudents[j]["Days"][1]["Pass"] == False)):

									# print(dataUnitStudents[j])
									# print()

									link = f"https://coddy.t8s.ru/Learner/{dataUnitStudents[j]['EdUnitType']}/{dataUnitStudents[j]['EdUnitId']}"
									tempDict = dict(StudentClientId=dataUnitStudents[j]["StudentClientId"], StudentName=dataUnitStudents[j]["StudentName"], EdUnitId= dataUnitStudents[j]["EdUnitId"], EdUnitLink = link)
